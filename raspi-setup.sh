#!/bin/sh
# Part of raspi-setup https://github.com/miconof/raspi-setup
#
...
on a Raspberry Pi. Although it can be run at any time, some of the\
options may have difficulties if you have heavily customised your\
installation. Note this is being used to configure a Raspbmc image, but\
most options should apply to any debian based image.\
" 20 70 1
}
...
# Opens and scans file in $2, looks for a matching line containing $1
get_config_var() {
  lua - "$1" "$2" <<EOF
local key=assert(arg[1])
...
# $1 is 0 to disable overscan, 1 to enable it
set_overscan() {
  # Stop if /boot is not a mountpoint
  if ! mountpoint -q /boot; then
    return 1
...
do_update() {
  printf "Pull from git repo and reexecute\n"
}
...
do
  ...
  *)
    # unknown option
    printf "No command line options currently supported\n"
    exit 1
    ;;
  esac
done
